...
# hydro
# list_of_cols_map = [
#     'date_last_researched', 'country', 'country_2', 'project_name', 
#     'project_name_(local_lang/script)', 'other_name(s)', 'capacity_(mw)', 
#     'binational', 'country_1_capacity_(mw)', 'country_2_capacity_(mw)', 
#     'turbines', 'status', 'complex', 'start_year', 'retired_year', 
#     'owner', 'owner_name_(local_lang/script)', 'operator', 
#     'operator_name_(local_lang/script)', 'technology_type', 
#     'river_/_watercourse', 'lat', 'lng', 'location_accuracy', 
#     'city_1', 'local_area_1', 'major_area_1', 'state/province_1', 
#     'subregion_1', 'region_1', 'city_2', 'local_area_2', 'major_area_2', 
#     'state/province_2', 'subregion_2', 'region_2', 'gem_location_id', 
#     'gem_unit_id', 'url']
# # rename columns
# # hydro rename
# df.rename(columns=lambda x: x.replace(' ', '_'), inplace=True)
# df.rename(columns=lambda x: x.lower(), inplace=True)
# df = df.rename(columns={'country_1': 'country', 'latitude':'lat', 'longitude':'lng', 'wiki_url': 'url'})
# df = df[list_of_cols_map]
# print(df.columns)


# TODO 
# create column that combines country 1 and 2
# as well as subnat? or skip
# separated by ;
# then use that to display all countries like we do for others
# and create a new column to display the map when more than one country